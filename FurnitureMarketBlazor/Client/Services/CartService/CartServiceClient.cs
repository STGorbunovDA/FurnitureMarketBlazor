...
namespace FurnitureMarketBlazor.Client.Services.CartService
{
    public class CartServiceClient : ICartServiceClient
    {
        ...
        public CartServiceClient(ILocalStorageService localStorage, HttpClient http, AuthenticationStateProvider authStateProvider) =>
                                (_localStorage, _http, _authStateProvider) = (localStorage, http, authStateProvider);
        ...
        private async Task<bool> IsUserAuthenticated()
        {
            ...
            // Возвращается значение, указывающее, аутентифицирован ли пользователь, используя свойство Identity.IsAuthenticated,
            // которое возвращает true, если идентификация пользователя прошла успешно, иначе возвращает false
            return authenticationState.User.Identity.IsAuthenticated;
        }

        public async Task GetCartItemsCount()
        {
            // Проверяем, аутентифицирован ли пользователь, вызывая метод IsUserAuthenticated(), который возвращает true или false,
            ...
            if (await IsUserAuthenticated())
            {
                ...
                // который будет десериализован в объект типа ServiceResponse<int> с помощью метода GetFromJsonAsync<T>()
                var result = await _http.GetFromJsonAsync<ServiceResponse<int>>("api/cart/count");

                // Извлекаем значение поля Data из полученного объекта result
                var count = result.Data;

                // Сохраняем значение count в локальное хранилище с ключом "cartItemsCount" с помощью метода SetItemAsync<T>()
            ...
            else
            {
                ...
                // Вычисляем количество элементов в корзине. Если cart равен null, то устанавливаем значение count равным 0,
        ...
        public async Task<List<CartProductResponse>> GetCartProducts()
        {
            ...
            else
            {
                ...
                if (cartItems == null)
                {
                    // Если список элементов корзины пуст, возвращаем новый пустой список типа List<CartProductResponse>
                ...
                // Читаем содержимое ответа в формате JSON и десериализуем его в объект типа ServiceResponse<List<CartProductResponse>>
                // с помощью метода ReadFromJsonAsync<T>() для доступа к данным (свойство Data)
                var cartProducts =
        ...
        public async Task RemoveProductFromCart(int productId, int productTypeId)
        {
            ...
            {
                // Если пользователь аутентифицирован, выполняем следующие действия
                // Отправляем DELETE-запрос по адресу $"api/cart/{productId}/{productTypeId}" с помощью метода DeleteAsync()
        ...
        public async Task StoreCartItems(bool emptyLocalCart = false)
        {
            ...
            // Отправляем POST-запрос по адресу "api/cart" и передаем localCart в качестве JSON-данных с помощью метода PostAsJsonAsync()
        ...
        public async Task UpdateQuantity(CartProductResponse product)
        {
            ...
            if (await IsUserAuthenticated())
            {
                // Если пользователь аутентифицирован, создаем объект CartItem с полями ProductId, Quantity и ProductTypeId
                ...
                // Отправляем PUT-запрос по адресу "api/cart/update-quantity" и передаем request в качестве JSON-данных
                // с помощью метода PutAsJsonAsync()
                await _http.PutAsJsonAsync("api/cart/update-quantity", request);
            }
            else
            {
                // Если пользователь не аутентифицирован, получаем список элементов корзины из локального хранилища с ключом "cart"
                var cart = await _localStorage.GetItemAsync<List<CartItem>>("cart");
                // Проверяем, равен ли cart null (т.е. список элементов корзины пуст)
                if (cart == null)
                {
